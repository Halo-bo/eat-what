...
async function rollDice() {
  ...
  try {
    ...
    for (let step = 0; step < totalSteps; step += 1) {
      ...
      positionMascot(true);
      updateCenterStage(getCurrentFood(), "walking");
      updateMysticSignal(null, "walking");
      await wait(310);
    }
...
function buildMysticSignal(food, status, meta = {}) {
  if (status === "walking") return "觅食中。。。";

  if (!food) {
    if (status === "spinning") return "觅食中。。。";
  ...
  const profile = getMysticProfile(food);
  const foodBoost = getFoodBoost(food);
  const fortuneBoost = getFortuneBoost(food, meta.spinValue);

  return `${spinText}停在${positionLabel}「${food.name}」：${profile.element}气${profile.motion}，主${profile.benefit}。${foodBoost}${fortuneBoost}`;
}

function getMysticProfile(food) {
  const profiles = {
    sweet: {
      element: "木火",
      motion: "生发",
  ...
  const tagOverrides = {
    清爽: { element: "金水", motion: "润下", benefit: "烦躁退潮" },
    清凉: { element: "水气", motion: "润下", benefit: "火气归零" },
    润润: { element: "白金", motion: "收敛", benefit: "情绪顺毛" },
    温柔: { element: "土气", motion: "安中", benefit: "内心落座" },
    沸腾: { element: "火土", motion: "生旺", benefit: "元气开锅" },
    醒脑: { element: "火气", motion: "升腾", benefit: "专注归位" },
    醒神: { element: "木气", motion: "生发", benefit: "灵感冒头" },
    补脑: { element: "金木", motion: "定盘", benefit: "思路收束" },
  };
  return tagOverrides[food.tag] || profiles[food.kind] || profiles.fun;
}

function getFoodBoost(food) {
  const boosts = {
    草莓: "桃花运+1，嘴角回春。",
    橙子: "低电量补阳，精神开窗。",
    冰激凌: "火气下线，暴躁退潮。",
  ...
  return boosts[food.name] || `${food.tag}入局，顺气+1。`;
}

function getFortuneBoost(food, spinValue) {
  const lucky = spinValue || ((state.position + food.name.length) % 8) + 1;
  const weatherTone = {
    hot: "热天调气",
    rain: "雨天稳盘",
    cool: "微凉生旺",
    mild: "平稳推运",
  }[state.weather?.mood] || "顺势补签";
  return `${weatherTone}，幸运数 ${lucky} 稳签。`;
}
